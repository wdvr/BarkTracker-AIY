...
DUMMY_META = to_didl_string(DidlItem(title="DUMMY", parent_id="DUMMY",item_id="DUMMY", desc='DUMMY'))
...
if __name__ == '__main__':
    ...
    for speaker in speakers:
        ...
        print("Found Sonos device on IP {0}, with name {1}. ".format(speaker.ip_address, speaker.player_name))
    ...
    while response.lower() not in ["y","yes","n","no"]:
        response = input("Play a short sample of your first favorite ({0}) on {1}?".format(favorites[0].title, first_speaker.player_name))
